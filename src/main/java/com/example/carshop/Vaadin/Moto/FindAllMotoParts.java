...
public class FindAllMotoParts extends VerticalLayout {
    ...
    public FindAllMotoParts(MotoService motoService) {
        ...
        motoGrid = new Grid<>(MotoDto.class);
        motoGrid.setColumns("mark", "model", "serialNumber", "partsBrand", "price", "quantity", "category");
        motoGrid.addColumn(new ComponentRenderer<>(this::fileReader)).setHeader("File");


        Set<MotoDto> serialNumber = findAllCarsBySerialNumber("wpisz nr seryjny");
        motoGrid.setItems(serialNumber);
        ...
        add(serialNumberField, searchButton, motoGrid,navigationLayout, buttonReturn);
